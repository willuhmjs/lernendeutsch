...
- **Model Selection for Self-Hosting:** With 96GB of VRAM, we have choices:
  ...
  - *Recommendation:* Run a single instance of a high-quality quantized model (like Llama-3-70B-Instruct) to ensure the best educational quality, relying on queueing and caching to manage load.
...
*Note: If a non-premium user provides their own API key (BYOK), their requests skip the local queue and go directly to their specified provider.*
...
- Show an upsell/informational banner to non-premium users: *"Experiencing slow responses? Our servers are currently under heavy load. Unlock priority queue access by signing up with a .edu email, or provide your own API key in Settings to bypass the queue entirely."*
