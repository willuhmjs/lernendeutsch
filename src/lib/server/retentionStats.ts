...
export async function loadRetentionStats(userId: string): Promise<RetentionStats> {
	...
	const retentionBuckets: RetentionBucket[] = retentionBucketDefs.map((b) => ({
		label: b.label,
		count: 0
	}));
	...
	const stabilityBuckets: RetentionBucket[] = stabilityBucketDefs.map((b) => ({
		label: b.label,
		count: 0
	}));
	...
	const forgettingCurve = [1, 3, 7, 14, 30].map((days) => ({
		days,
		retentionPct:
			medianStability > 0 ? Math.round(Math.pow(1 + days / (9 * medianStability), -1) * 100) : 0
	}));
