<script lang="ts">
	...
	function startEditQuestion(question: any) {
		...
		// Pad to 3 options
		editOptions = [optionsArray[0] || '', optionsArray[1] || '', optionsArray[2] || ''];
	}
...
<div class="editor-container">
	<div class="header-section">
		...
		<div class="header-actions">
			<button
				...
				class="btn-toggle {isPublished ? 'published' : 'draft'} {isPublishing ? 'publishing' : ''}"
			>
				{isPublishing ? 'Publishing...' : isPublished ? 'Published' : 'Draft'}
			</button>
			<button onclick={deleteGame} disabled={isPublishing} class="btn-delete"> Delete Quiz </button>
		</div>
	</div>

	<!-- Quiz Details Section -->
	<div
		class="card-duo details-card"
		style={isPublishing ? 'opacity: 0.6; pointer-events: none;' : ''}
	>
		...
		{#if isEditingDetails}
			<div class="edit-form">
				...
				<button onclick={saveGameDetails} disabled={isPublishing} class="btn-primary save-btn"
					>Save Details</button
				>
			</div>
		{:else}
	...
	<div class="card-duo ai-card" style={isPublishing ? 'opacity: 0.6; pointer-events: none;' : ''}>
		...
		<p class="ai-desc">Use AI to instantly create vocabulary or grammar questions for this quiz.</p>
		<div class="ai-form">
			<input
				type="text"
	...
	{#if isAddingQuestion}
		<div
			class="card-duo add-question-card"
			style={isPublishing ? 'opacity: 0.6; pointer-events: none;' : ''}
		>
			<h3>New Question</h3>
			<div class="edit-form">
				<div class="field">
					...
					<input
						...
						disabled={isPublishing}
						placeholder="e.g. Translate 'Hello'"
						class="form-input"
					/>
				</div>
				<div class="field">
				...
				<div class="field">
					...
					<div class="options-grid">
						...
						<input
							type="text"
							bind:value={newOptions[1]}
							disabled={isPublishing}
							placeholder="Distractor 2"
							class="form-input"
							aria-label="Distractor 2"
						/>
						<input
							type="text"
							bind:value={newOptions[2]}
							disabled={isPublishing}
							placeholder="Distractor 3"
							class="form-input"
							aria-label="Distractor 3"
						/>
					</div>
				</div>
				<button onclick={addQuestion} disabled={isPublishing} class="btn-primary save-btn mt-2"
						...
						<div class="field">
							...
							<div class="options-grid">
								<input
									id="edit-option-1"
									type="text"
									bind:value={editOptions[0]}
									class="form-input"
									aria-label="Distractor 1"
								/>
								<input
									type="text"
									bind:value={editOptions[1]}
									class="form-input"
									aria-label="Distractor 2"
								/>
								<input
									type="text"
									bind:value={editOptions[2]}
									class="form-input"
									aria-label="Distractor 3"
								/>
							</div>
